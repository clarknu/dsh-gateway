 ...
 * @param upstream - loopback http URL of the dsh webserver
 * @param {{ upstreamTimeoutMs?: number }} [opts] - `upstreamTimeoutMs` caps one
 *   proxied request/response turn (default 30s) so a wedged upstream surfaces
 *   as 502 instead of hanging the client forever.
 */
export function createProxy(upstream, opts = {}) {
  const upstreamTimeoutMs = opts.upstreamTimeoutMs ?? 30000
  ...
  const forwardHeaders = (req) => {
    ...
    // Preserve the client's Host header (transparent proxy). The dsh
    // trust fence requires Origin.host === Host.host, so rewriting Host to
    // the upstream loopback address would 403 every browser POST and
    // WebSocket handshake. Only HTTP/1.0 clients without a Host get the
    // upstream authority synthesized.
  ...
  function handleRequest(req, res) {
    ...
    // Abandoned client (nginx timeout, tab close): destroy the upstream work,
    // otherwise a streamed response keeps its pooled keepalive socket busy
    // forever and the abandoned turn is never torn down.
    res.on('close', () => upstreamReq.destroy())
    // A wedged upstream turn must surface as 502, not hang the client forever.
    upstreamReq.setTimeout(upstreamTimeoutMs, () =>
      upstreamReq.destroy(new Error(`gateway: upstream read timeout after ${upstreamTimeoutMs}ms`)),
    )
