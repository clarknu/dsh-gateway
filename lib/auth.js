...
// per-IP login-failure lockout, and the login page. The gateway checks the
// cookie itself on every request.
...
/**
 * Self-describing scrypt hash format stored in `gateway.users` values:
 * `scrypt$N$r$p$salt$hash` (base64url, no padding). Detected by
 * {@link verifyPassword}; anything else is treated as the legacy plaintext
 * value (still verified, but users are warned to migrate).
 */
...
/**
 * Constant-time verification against a stored value: a `scrypt$...` hash or a
 * legacy plaintext password (kept for backward compatibility). Malformed or
 * unverifiable stored values fail closed.
 */
export function verifyPassword(password, stored) {
  ...
  if (s.startsWith(SCRYPT_PREFIX)) {
    ...
    const rn = Number(r)
    const pn = Number(p)
    if (!Number.isInteger(Nn) || !Number.isInteger(rn) || !Number.isInteger(pn) || Nn <= 0 || rn <= 0 || pn <= 0) return false
    let salt
    let expected
    try {
    ...
    try {
      const derived = scryptSync(String(password), salt, expected.length, { N: Nn, r: rn, p: pn })
      return safeEqual(derived, expected)
    } catch {
      return false // oversized cost parameters / maxmem exceeded → fail closed
    }
  }
  // Legacy plaintext value: constant-time compare of the sha256 digests.
  // Equivalent in strength to the previous scheme; boot warns to migrate.
...
export function createAuth(options) {
  // The signing secret is read live from `options.hmacSecret` on every
  // sign/verify, so rotating the value (e.g. a "revoke all sessions" action)
  // invalidates every previously issued cookie immediately — no listener
  // restart needed. The fallback only exists for standalone use without a
  // supplied secret.
  const fallbackSecret = options.hmacSecret ?? randomBytes(32).toString('base64')
  const getSecret = () => options.hmacSecret ?? fallbackSecret
  ...
  /** Constant-time password check against the user table (scrypt or legacy). */
  function checkCredentials(username, password) {
    const expected = (options.users ?? {})[username]
    if (expected === undefined) return false
    return verifyPassword(password, expected)
  }
